...
def validate_skill(skill_path):
    ...
    runtime_schema = repo_root / 'schemas' / 'runtime-profile.schema.json'
    if not runtime_schema.exists():
        return False, "Missing schemas/runtime-profile.schema.json"

    try:
        schema_data = json.loads(runtime_schema.read_text())
    except json.JSONDecodeError as e:
        return False, f"Invalid JSON in runtime-profile.schema.json: {e}"

    if '$schema' not in schema_data or 'title' not in schema_data:
        return False, "runtime-profile.schema.json must contain $schema and title"
